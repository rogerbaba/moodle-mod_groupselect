 ...
 *
 * @param $groupselect The group select activity
 * @param $userid      The user id
 * @return bool        True if enrolled in any group, false if not enrolled in any group
 * @throws dml_exception
 */
function groupselect_get_user_answer($groupselect, $userid) {
    global $DB, $COURSE;

    // Get the groups in this group select.
    $groupselectgroups = [];
    // Is there a grouping?
    $groupingid = $DB->get_field('groupselect', 'targetgrouping', array('id' => $groupselect->id));
    ...
    if (empty($groupingid)) {
        ...
        $groupselectgroups = $DB->get_records_sql($sql, $params);
    } else {
        $sql = 'SELECT g.id
              FROM {groups} g
